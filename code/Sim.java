...
/**
 * Simulates priority queueing in a router to order packets.
 */
...
public class Sim {
	...
	private static final int OUT_ORDER_MAX = 10 * BYTES_TO_BITS * MEGA_TO_UNIT;
	private static final int IN_ORDER_MAX = 10 * BYTES_TO_BITS * MEGA_TO_UNIT;
	...
	// router
	private static Router router_queue;
	...
	private static long number_of_customers, number_of_finished, in_order_router, in_order_destination, current_seq_destination, dropped, router_arrivals;
	...
	private static void initialize() {
		...
		source_delay = new RandomVariate(stream, 0.2, 0.2);
	...
	public static void main(String argv[]) {
		router_queue = new Router(OUT_ORDER_MAX, IN_ORDER_MAX);
	...
	private static void generateReport() {
		...
		//System.out.printf("DOUBLE QUEUE ROUTER SIMULATION - PACKET REORDERING WITH PRIORITY QUEUEING\n");
		...
		//System.out.printf("Percent in order (router):                             %10.2f%%\n", percentage_in_order_router);
		System.out.printf("Percent in order (destination):                        %10.2f%%\n", 100-percentage_in_order_destination);
	...
	private static void processRouterArrival(Event e) {
		router_arrivals++;
		int queue = router_queue.packetEnqueue(e.getPacket());
		e.getPacket().begin_queueing = clock;

		if (queue == 0) dropped++;
		else if (queue == 1) in_order_router++;

		if (!in_service) {
			Packet p = router_queue.packetTransmit();
